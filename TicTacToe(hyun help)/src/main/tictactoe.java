...
public class tictactoe {
    ...
    public int[] bestMove(String[][] board, int boardsize, int current) {
    	...
    	int[] currentCorPlusRating = new int[3];
    	
    	for (int i = 0; i < 1; i++) {
    		currentCorPlusRating[i] = currentCor[i];
    	}
    	
    	currentCorPlusRating[2] = rating;

        //changePlayer();
        return currentCorPlusRating;
        ...
        while (true) {
            ...
            if(vsCpuBool) {
            	...
            	if(canWin(boardsize)) {
            		...
            		String[][] oneStepFutureBoard = copyBoard(board, boardsize);
            		int highestRating = -((2^31)-1);
            		...
            		for(int i = 0; i < boardsize;i++) {
            			for(int j = 0; j < boardsize;j++) {
            				...
            				if (!(i < 0 || j < 0 || i >= boardsize || j >= boardsize || board[i][j] != "-")) {
            					...
            					oneStepFutureBoard[i][j] = currentPlayer;
            					int[] thisMove = bestMove(oneStepFutureBoard, boardsize,0);
            					
            					if(thisMove[2] > highestRating) {
            						
            						thisMove[2] = highestRating;
            						bestMove[0] = thisMove[0];
            						bestMove[1] = thisMove[1];
            				...
            				oneStepFutureBoard = copyBoard(board, boardsize);
            				
            			}
            		}
            		
            		int[] winningCor = bestMove;
            		row = winningCor[0];
            		col = winningCor[1];
